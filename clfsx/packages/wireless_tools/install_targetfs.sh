#!/bin/bash
## @file wireless_tools/install_targetfs.sh
## @brief builds wireless tools and copies executables to the targetfs directory
...
touch $LOG
chmod ugo+rw $LOG
echo "compiling wireless tools"
make clean  > $LOG 2>&1
if [ $? -ne 0 ]; then echo "exiting $PACKAGE_NAME $STEP due to make failure during configure"; exit $?; fi
make BUILD_STATIC="y" PREFIX="${CLFS_TARGETFS_DIR}" CC="${CC}"  > $LOG 2>&1
if [ $? -ne 0 ]; then echo "exiting $PACKAGE_NAME $STEP due to make failure during configure"; exit $?; fi
echo "installing wireless tools"
make BUILD_STATIC="y" PREFIX="${CLFS_TARGETFS_DIR}" CC="${CC}" install  > $LOG 2>&1
if [ $? -ne 0 ]; then echo "exiting $PACKAGE_NAME $STEP due to make failure during configure"; exit $?; fi
