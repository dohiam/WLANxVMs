#!/bin/bash
## @file tcpdump/install_targetfs.sh
## @brief builds tcpdump and copies executables to the targetfs directory
...
./configure --build="${CLFS_HOST}" --host="${CLFS_HOST}" --target="${CLFS_TARGET}" --prefix="${CLFS_TARGETFS_DIR}"  > $LOG 2>&1
