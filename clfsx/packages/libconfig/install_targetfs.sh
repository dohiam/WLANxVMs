#!/bin/bash
## @file libconfig/install_targetfs.sh
## @brief builds libconfig and copies executables to the targetfs directory
...
touch $LOG
chmod ugo+rw $LOG
echo "configuring libconfig"
./configure --build="${CLFS_HOST}" --host="${CLFS_HOST}" --target="${CLFS_TARGET}" --prefix="${CLFS_BUILD_DIR}/libconfig-install"  > $LOG 2>&1
...
if [ $? -ne 0 ]; then echo "exiting $PACKAGE_NAME $STEP due to make failure during configure"; exit $?; fi
cp -r ${CLFS_BUILD_DIR}/libconfig-install/* ${CLFS_TARGETFS_DIR}
